...
function SideNav({ selected, onPanelChange }: SideNavProps) {
  ...
  return (
    <div className="h-full">
      ...
      <SideNavItem selected={selected === 'discoveries'} icon={<SearchAlert className='pb-1 w-16 h-16' />} label="Discoveries" onClick={() => onPanelChange('discoveries')} />
      <SideNavItem selected={selected === 'run_history'} icon={<History className='pb-1 w-16 h-16' />} label="Run History" onClick={() => onPanelChange('run_history')} />
