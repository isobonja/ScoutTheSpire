...
const debug_1$3 = __importDefault$3(srcExports$1);
...
var debug_1 = function() {
  if (!debug$1) {
    try {
      debug$1 = srcExports$1("follow-redirects");
    } catch (error) {
    }
...
followRedirects$1.exports = wrap({ http: http$1, https });
...
var DURATION_UNITS = { ms: 1, s: 1e3, m: 6e4, h: 36e5 };
...
function buildWindows(options) {
  ...
  if (limits && limits.length > 0) {
    return limits.map(function(limit, i) {
      var max2 = limit.maxRequests;
      if (typeof max2 !== "number" || !isFinite(max2) || max2 <= 0) {
        throw new Error(
          "Invalid rate limit option at limits[" + i + "]: maxRequests is required and must be a positive number."
        );
      }
      var perMs2 = parseDuration(limit.duration);
      if (typeof perMs2 !== "number" || !isFinite(perMs2) || perMs2 <= 0) {
        throw new Error(
          "Invalid rate limit option at limits[" + i + "]: duration must be a positive finite number."
        );
      }
      return { count: 0, max: max2, perMs: perMs2, timeoutId: null };
    });
  }
  ...
  if (typeof perMs !== "number" || !isFinite(perMs) || perMs <= 0) {
    throw new Error(
      "Invalid rate limit options: one of maxRPS, duration, or perMilliseconds is required and must be positive."
    );
  }
  var maxInvalid = typeof maxRequests !== "number" || !isFinite(maxRequests) || maxRequests <= 0;
  if (maxInvalid) {
    throw new Error(
      "Invalid rate limit options: maxRequests is required and must be a positive number."
    );
  }
  return [{ count: 0, max: maxRequests, perMs, timeoutId: null }];
}
...
AxiosRateLimit.prototype.enable = function(axios2) {
  ...
  function handleError(error) {
    self2.shift().catch(function() {
    });
    return Promise.reject(error);
  }
  axios2.interceptors.request.use(
    function(request) {
      return self2.handleRequest(request);
    },
    function(error) {
      return Promise.reject(error);
    }
  ...
  axios2.interceptors.response.use(
    function(response) {
      return self2.handleResponse(response);
    },
    handleError
  );
  axios2.getQueue = function() {
    return self2.getQueue();
  };
  axios2.getMaxRPS = function() {
    return self2.getMaxRPS();
  };
  axios2.setMaxRPS = function(rps) {
    self2.setMaxRPS(rps);
  };
  axios2.setRateLimitOptions = function(options) {
    self2.setRateLimitOptions(options);
  };
  return axios2;
};
...
AxiosRateLimit.prototype.handleRequest = function(request) {
  ...
  return new Promise(function(resolve2, reject) {
    ...
    Promise.resolve(self2.queue.push(handler)).then(function() {
      self2.shiftInitial();
    }).catch(reject);
  });
};
AxiosRateLimit.prototype.handleResponse = function(response) {
  var self2 = this;
  if (typeof self2._shouldCountRequest === "function") {
    try {
      if (self2._shouldCountRequest(response.config, response) === false) {
        for (var i = 0; i < self2.windows.length; i++) {
          var w = self2.windows[i];
  ...
  return Promise.resolve(self2.shift()).then(function() {
    return response;
  });
};
...
AxiosRateLimit.prototype.shift = function() {
  var self2 = this;
  function doShift() {
    return getQueueLength(self2.queue).then(function(len) {
      if (!len) return void 0;
      var windows = self2.windows;
      for (var i = 0; i < windows.length; i++) {
      ...
      return Promise.resolve(self2.queue.shift()).then(function(queued) {
        if (queued == null) return void 0;
        ...
        for (var j = 0; j < windows.length; j++) {
          ...
          w.count += 1;
          if (w.count === 1) {
            w.timeoutId = setTimeout((function(win2) {
              win2.count = 0;
              win2.timeoutId = null;
              function next() {
                getQueueLength(self2.queue).then(function(queueLen) {
                  if (!queueLen) return;
                  var wins = self2.windows;
                  var blocked = false;
                  ...
                  if (blocked) return;
                  self2.shift().then(next).catch(function() {
                  });
                });
              }
              next();
  ...
  if (isAsyncQueue(self2.queue)) {
    var p = self2._shiftPromise.then(doShift);
    self2._shiftPromise = p.catch(function() {
    });
    return p;
  }
  return doShift();
...
function axiosRateLimit(axios2, options) {
  ...
  return rateLimitInstance.enable(axios2);
}
...
src.exports = axiosRateLimit;
src.exports.AxiosRateLimiter = AxiosRateLimit;
...
async function fetchEpochsData() {
  try {
